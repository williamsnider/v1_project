...
void pushMergedPresynapticUpdateGroup0ToDevice(unsigned int idx, float* inSyn, unsigned int* srcSpkCnt, unsigned int* srcSpk, unsigned int* trgSpkQuePtr, unsigned int* rowLength, uint32_t* ind, unsigned int rowStride, unsigned int numSrcNeurons, unsigned int numTrgNeurons) {
    MergedPresynapticUpdateGroup0 group = {inSyn, srcSpkCnt, srcSpk, trgSpkQuePtr, rowLength, ind, rowStride, numSrcNeurons, numTrgNeurons, };
...
void pushMergedPresynapticUpdateGroup1ToDevice(unsigned int idx, float* inSyn, unsigned int* srcSpkCnt, unsigned int* srcSpk, unsigned int* srcSpkQuePtr, unsigned int* trgSpkQuePtr, unsigned int* rowLength, uint32_t* ind, unsigned int rowStride, unsigned int numSrcNeurons, unsigned int numTrgNeurons) {
    MergedPresynapticUpdateGroup1 group = {inSyn, srcSpkCnt, srcSpk, srcSpkQuePtr, trgSpkQuePtr, rowLength, ind, rowStride, numSrcNeurons, numTrgNeurons, };
                ...
                for (unsigned int j = 0; j < numSpikesInBlock; j++) {
                    ...
                    if (lid < group->rowStride) {
                        ...
                        if (lid < npost) {
                            ...
                            atomicAdd(&group->inSyn[ipost], (-2.00000000000000011e-01f));
                        }
                    }
                }
            }
