...
def build_v1_network():
    ...
    for i, model_props in enumerate(glif_models[:1]):
        n_cells = 1 if model_props["ei"] == "e" else 1  # 80% excitatory, 20% inhib

        # Randomly get positions uniformly distributed in a column
        positions = positions_columinar(
            N=n_cells, center=[0, 10.0, 0], max_radius=50.0, height=200.0
        )
        ...
        v1.add_nodes(
            ...
            y=positions[:, 1],
            z=positions[:, 2],
            rotation_angle_yaxis=xiter_random(
                N=n_cells, min_x=0.0, max_x=2 * np.pi
    ...
    #     n_cells = (
    #         75  # Just assume 75 cells for both point inhibitory and point excitatory
    #     )
    #     positions = positions_columinar(
    #         N=n_cells, center=[0, 10.0, 0], max_radius=50.0, height=200.0
    #     )
    ...
    def n_connections(src, trg, prob=0.1, min_syns=1, max_syns=5):
        ...
        return (
            0 if np.random.uniform() > prob else np.random.randint(min_syns, max_syns)
        )
...
def build_lgn_network(v1):
    ...
    lgn.add_nodes(N=1, model_type="virtual", ei="e")

    # Targets all glif excitatory cells
    lgn.add_edges(
        target=v1.nodes(ei="e", placement="inner"),
        source=lgn.nodes(),
        connection_rule=lambda *_: 2,  # CHECK THIS: #np.random.randint(0, 5),
    ...
    # lgn.add_edges(
    #     target=v1.nodes(ei="i", placement="inner"),
    #     source=lgn.nodes(),
    ...
    # lgn.add_edges(
    #     target=v1.nodes(placement="outer"),
    #     source=lgn.nodes(),
    #     connection_rule=lambda *_: np.random.randint(0, 5),
    #     dynamics_params="instanteneousExc.json",
    #     model_template="static_synapse",
