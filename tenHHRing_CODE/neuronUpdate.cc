...
void pushMergedNeuronUpdateGroup0ToDevice(unsigned int idx, unsigned int* spkCnt, unsigned int* spk, unsigned int* spkQuePtr, scalar* V, scalar* m, scalar* h, scalar* n, float* inSynInSyn0, float* inSynInSyn1, unsigned int numNeurons) {
    MergedNeuronUpdateGroup0 group = {spkCnt, spk, spkQuePtr, V, m, h, n, inSynInSyn0, inSynInSyn1, numNeurons, };
...
void pushMergedNeuronUpdateGroup1ToDevice(unsigned int idx, unsigned int* spkCnt, unsigned int* spk, unsigned int* startSpike, unsigned int* endSpike, scalar* spikeTimes, unsigned int numNeurons) {
    MergedNeuronUpdateGroup1 group = {spkCnt, spk, startSpike, endSpike, spikeTimes, numNeurons, };
...
void pushMergedNeuronUpdate1spikeTimesToDevice(unsigned int idx, scalar* value) {
    CHECK_CUDA_ERRORS(cudaMemcpyToSymbolAsync(d_mergedNeuronUpdateGroup1, &value, sizeof(value), (sizeof(MergedNeuronUpdateGroup1) * (idx)) + offsetof(MergedNeuronUpdateGroup1, spikeTimes)));
}
